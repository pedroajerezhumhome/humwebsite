import type { Metadata } from "next";
import Hero from "@/components/precall/Hero";
import ProductShowcase from "@/components/precall/ProductShowcase";
import CostCalculator from "@/components/CostCalculator";
import PrecallFooter from "@/components/precall/PrecallFooter";
import FAQ from "@/components/precall/FAQ";
import ScrollReveal from "@/components/precall/ScrollReveal";
import ScrollProgress from "@/components/precall/ScrollProgress";
import HelloBar from "@/components/precall/HelloBar";
import StickyNav from "@/components/precall/StickyNav";
import "../../precall/precall.css";
import "@/app/calculator/calculator.css";

export const metadata: Metadata = {
  title: "HUM: Before Your Call",
  description:
    "Your personalized pre-call guide. Everything you need to know before your HUM discovery call.",
};

export default function PrecallMomPage() {
  return (
    <div className="precall-page">
      <ScrollReveal />
      <ScrollProgress />
      <HelloBar />
      <StickyNav />

      <Hero
        headline="Everything you need to know before your call"
        trustText='Trusted by <strong>100+ families</strong> across the country. Your call is a conversation, not a commitment.'
      />

      <p className="urgency-nudge rv">
        Families who read this before their call tell us the conversation goes deeper.
        Five minutes now saves you thirty on the phone.
      </p>

      {/* Your life right now */}
      <section className="sect rv">
        <div className="narrow">
          <div className="label">Your life right now</div>
          <h2>You already know how tomorrow is going to go</h2>
          <p>
            You&rsquo;re up before everyone else. Packing lunches, managing the
            chaos of getting everyone fed, dressed, and out the door, while
            answering a work email that can&rsquo;t wait. Then the evening.
            Dinner, homework, baths, bedtime. All while running through
            tomorrow&rsquo;s logistics.
          </p>
          <p>
            Your phone never shuts off. Nobody else is going to notice the kids
            are outgrowing their shoes. Nobody else is going to remember the
            permission slip is due Friday. Nobody else is going to think about
            what&rsquo;s for dinner on Tuesday night at 2 PM on a Monday.
          </p>
          <p>
            You lie in bed at night running the audit. Did the permission slip
            get signed? Is there enough milk? When was the last time the dog had
            his heartworm pill? The household never shuts down. It works because
            you never stop.
          </p>
          <p style={{ fontSize: "0.9rem", color: "var(--text-muted)" }}>
            Research shows mothers handle 71% of all household cognitive labor.
            Not just the doing. The thinking about the doing.
          </p>
          <div className="pullquote">
            You can&rsquo;t drop the thread. Because if you do, everything
            unravels. So you never do.
          </div>

          <p style={{ fontSize: "0.95rem", color: "var(--text)", fontWeight: 500, marginTop: 28 }}>
            No family should run on a mother&rsquo;s burnout.
          </p>

          <p className="fwd-hook">This is what you decided to change.</p>
        </div>
      </section>

      {/* Cost of staying here */}
      <section className="sect dark rv">
        <div className="narrow">
          <div className="label">Why the timing is right</div>
          <h2>One year from now</h2>
          <p>
            Same pace. Same weight. Same list. Twelve more months of managing
            instead of living. Your career never got the bandwidth it
            deserved. Your relationship is another year more threadbare, not
            because the love disappeared, but because every conversation became a
            logistics meeting.
          </p>
          <p>
            And your kids are another year closer to not needing bedtime
            stories and Saturday morning pancakes. The window is closing.
            It doesn&rsquo;t reopen.
          </p>
          <div className="pullquote">
            You lose those years in 30-minute increments, folding laundry,
            coordinating vendors, making tomorrow&rsquo;s list, while the life
            you wanted was happening in the next room.
          </div>
        </div>
      </section>

      {/* Permission */}
      <section className="sect rv">
        <div className="narrow">
          <h2>You&rsquo;re allowed to want your life back</h2>
          <p>
            If there&rsquo;s a voice saying you should be able to handle this,
            that wanting help means you&rsquo;re not enough, here&rsquo;s the
            truth. You are carrying more than any one person should. Not because
            you signed up for it. Because nobody else picked it up.
          </p>
          <p>
            And if part of you feels guilty for having high standards, for wanting
            things done a certain way, you&rsquo;re not being too much.
            Everything you want for your home is realistic. You just
            haven&rsquo;t had the right help yet.
          </p>
          <div className="pullquote">
            Wanting your life back is not a luxury. It&rsquo;s clarity.
          </div>
        </div>
      </section>

      {/* Prepare */}
      <section className="sect warm rv">
        <div className="container">
          <div className="narrow" style={{ textAlign: "center" }}>
            <div className="label">Before your call</div>
            <h2>Three things to think about</h2>
            <p style={{ color: "var(--text-soft)" }}>
              You don&rsquo;t need to prepare anything formal. But families
              who&rsquo;ve thought about these get the most from the
              conversation.
            </p>
          </div>
          <div
            className="prep-items"
            style={{ maxWidth: 720, marginLeft: "auto", marginRight: "auto" }}
          >
            <div className="prep-card rv rv-d1">
              <div className="prep-num">1</div>
              <h3>What&rsquo;s not working</h3>
              <p>
                What are the specific moments where the weight is heaviest?
                Morning chaos, evening scramble, weekends consumed by catch-up?
              </p>
            </div>
            <div className="prep-card rv rv-d2">
              <div className="prep-num">2</div>
              <h3>What you&rsquo;ve tried</h3>
              <p>
                Cleaners, nannies, apps, asking your partner. What solved a
                piece and what created more to manage?
              </p>
            </div>
            <div className="prep-card rv rv-d3">
              <div className="prep-num">3</div>
              <h3>What great looks like</h3>
              <p>
                If this worked perfectly, what does your Tuesday look like? Your
                Saturday? What do you do with the time you get back?
              </p>
            </div>
          </div>
        </div>
      </section>

      {/* After picture */}
      <section className="sect rv">
        <div className="narrow">
          <div className="label">What changes first</div>
          <h2>The morning you haven&rsquo;t had in years</h2>
          <p>
            You wake up and the house is already running. The lunches are made.
            The errands are in motion. Not because you remembered them,
            but because someone else did. You sit down with your coffee. Your
            morning is yours.
          </p>
          <p>
            The electrician comes and you don&rsquo;t know about it until
            it&rsquo;s done. The pantry is stocked. The returns got dropped off.
            You pick up your kids and you&rsquo;re actually present. Not
            distracted. Just there.
          </p>
          <p>
            Your daughter asks &ldquo;Can we bake cookies?&rdquo; and you say
            yes. Not because you found the time. Because the time was already
            there. Your partner says &ldquo;What should we do this weekend?&rdquo;
            and neither of you means chores.
          </p>
          <p>
            Saturday morning pancakes. A spontaneous trip to the park.
            You catch yourself laughing at the dinner table. Actually
            laughing. Because you&rsquo;re not running tomorrow&rsquo;s
            audit in your head.
          </p>
          <div className="pullquote">
            Same house. Same family. Same life. The only difference is that
            you&rsquo;re finally in it instead of managing it.
          </div>
          <p>
            You&rsquo;re the architect of your family&rsquo;s life, not the laborer.
            You didn&rsquo;t give up. You built the system.
          </p>
          <p className="fwd-hook">And if part of you is still wondering whether this time is different, that&rsquo;s worth reading too.</p>
        </div>
      </section>

      {/* Why everything else failed */}
      <section className="sect rv">
        <div className="narrow">
          <div className="label">What makes this different</div>
          <h2>You&rsquo;ve heard promises like this before</h2>
          <p>
            If part of you is reading this thinking &ldquo;that sounds like
            exactly what I need&rdquo;, and right behind that is &ldquo;I&rsquo;ve
            been burned before&rdquo;, good. That instinct has protected you.
          </p>
          <p>
            Most families who try to hire help on their own hit the same
            three walls:
          </p>
          <div style={{ display: "flex", flexDirection: "column", gap: 20, margin: "24px 0" }}>
            <div>
              <h3 style={{ fontSize: "1rem", marginBottom: 4 }}>Wrong role design</h3>
              <p style={{ color: "var(--text-soft)", marginBottom: 0 }}>
                Requirements are vague. The house manager doesn&rsquo;t know
                what to prioritize. Things fall through the cracks and the family
                feels like nothing is getting done right.
              </p>
            </div>
            <div>
              <h3 style={{ fontSize: "1rem", marginBottom: 4 }}>Wrong hiring process</h3>
              <p style={{ color: "var(--text-soft)", marginBottom: 0 }}>
                A generic job description brings the wrong applicants. Without
                screening filters, you spend hours on calls with people who
                aren&rsquo;t qualified. You hire on gut feel instead of fit, and
                it falls apart within weeks.
              </p>
            </div>
            <div>
              <h3 style={{ fontSize: "1rem", marginBottom: 4 }}>Wrong onboarding</h3>
              <p style={{ color: "var(--text-soft)", marginBottom: 0 }}>
                The new person shows up with nothing to go on. You spend
                weeks repeating yourself, micromanaging, and when it
                doesn&rsquo;t work out, you start completely over. No
                documentation carries forward.
              </p>
            </div>
          </div>
          <div className="pullquote">
            The solution isn&rsquo;t another person to manage. It&rsquo;s
            getting the foundation right first, then finding the right person
            to run it.
          </div>
          <p>That&rsquo;s what HUM builds. Here&rsquo;s how.</p>
        </div>
      </section>

      {/* Phase 1 deliverables — what you get */}
      <section id="how-it-works" className="sect warm rv">
        <div className="narrow">
          <div className="label">What you walk away with</div>
          <h2>Everything you need to hire the right person the first time</h2>
          <p>
            Before anyone starts searching, we do a deep dive into how your
            household actually runs. Then we build every tool you need, customized
            to your family. Nothing is a template.
          </p>
          <div className="grid-2col" style={{ margin: "28px 0" }}>
            <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 8, padding: 20 }}>
              <h3 style={{ fontSize: "0.95rem", marginBottom: 8 }}>Role design &amp; delegation strategy</h3>
              <p style={{ fontSize: "0.88rem", color: "var(--text-soft)", marginBottom: 0 }}>
                What the role is, what it isn&rsquo;t, service area priorities,
                schedule, compensation benchmarks for your market, and how to
                delegate effectively so your house manager knows the difference
                between the majors and the minors from day one.
              </p>
            </div>
            <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 8, padding: 20 }}>
              <h3 style={{ fontSize: "0.95rem", marginBottom: 8 }}>Hiring toolkit</h3>
              <p style={{ fontSize: "0.88rem", color: "var(--text-soft)", marginBottom: 0 }}>
                A platform-optimized job description written to attract the
                right candidates and repel the wrong ones. Screening questions
                that qualify before you ever get on a call. Interview guide with
                scoring rubric. Social media sourcing posts.
              </p>
            </div>
            <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 8, padding: 20 }}>
              <h3 style={{ fontSize: "0.95rem", marginBottom: 8 }}>Complete home manual</h3>
              <p style={{ fontSize: "0.88rem", color: "var(--text-soft)", marginBottom: 0 }}>
                A living guide covering how your household runs day to day.
                Step-by-step playbooks for every service area, emergency
                procedures, contact directory, product catalog, and
                communication preferences. Your house manager can follow it from
                day one without asking you how everything works.
              </p>
            </div>
            <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 8, padding: 20 }}>
              <h3 style={{ fontSize: "0.95rem", marginBottom: 8 }}>Onboarding, management &amp; compliance</h3>
              <p style={{ fontSize: "0.88rem", color: "var(--text-soft)", marginBottom: 0 }}>
                First-day walkthrough, 30/60-day check-in framework, how to
                give feedback and handle performance, off-boarding guide for
                clean transitions, and everything you need to be set up
                correctly as a household employer.
              </p>
            </div>
          </div>
          <div className="pullquote">
            You wouldn&rsquo;t hire someone at work without a job description,
            clear expectations, and an onboarding plan. Your home deserves
            the same.
          </div>
          <p style={{ fontSize: "0.88rem", color: "var(--text-muted)", marginTop: 20 }}>
            Already have SOPs, notes, or things you&rsquo;ve brainstormed? Send
            them over. We&rsquo;ll consolidate everything into one system
            and fill in the gaps.
          </p>
        </div>
      </section>

      <ProductShowcase
        label="What the system looks like"
        heading="The Home OS and Hiring Process"
        introText="Here's a preview of what we build and how the search works."
        footerNote="Your house manager can also serve as a nanny, housekeeper, or combination, depending on what your family needs. We'll tailor the role on your call."
      />

      {/* Home OS deep dive — turnover protection */}
      <section className="sect rv">
        <div className="narrow">
          <div className="label">Why the Home OS matters most</div>
          <h2>The part most families don&rsquo;t expect to be the most valuable</h2>
          <p>
            Here&rsquo;s something no one talks about: the question
            isn&rsquo;t <em>whether</em> your house manager will eventually
            move on. It&rsquo;s <em>when</em>. Even great house managers leave. Family reasons,
            relocation, career changes.
          </p>
          <p>
            Without a system, that means starting from scratch. Re-explaining every routine.
            Re-training every preference. Standing over someone&rsquo;s shoulder for weeks
            while your life goes back to chaos.
          </p>
          <p>
            The Home Operating System changes that equation completely. Everything about how
            your home runs is documented, organized, and accessible. Visual
            playbooks showing exactly how the pantry looks when stocked, how the kitchen gets
            reset, what the daily and weekly routines look like. When a new house manager steps
            in, they have full context from day one. No re-training. No lost momentum.
          </p>
          <div className="pullquote">
            You don&rsquo;t build any of this yourself. We extract it from you through the
            onboarding process. Before you even realize it, we&rsquo;re turning what&rsquo;s
            trapped in your head into a system that runs without you.
          </div>
          <p>
            And once it&rsquo;s built, there&rsquo;s an AI co-pilot inside it. Your house
            manager can ask &ldquo;Where do the soccer cleats go?&rdquo; or &ldquo;Which
            vendor does the family use for pest control?&rdquo; and get an answer instantly
            without texting you. The knowledge isn&rsquo;t trapped in your head
            anymore. It&rsquo;s in the system.
          </p>
        </div>
      </section>

      {/* What your house manager handles — Q5 + Q3 */}
      <section id="what-they-handle" className="sect dark rv">
        <div className="narrow">
          <div className="q-callout light">Families ask: &ldquo;What does a house manager actually do?&rdquo;</div>
          <div className="label">The role</div>
          <h2>Everything that&rsquo;s currently living in your head</h2>
          <p>
            Your house manager isn&rsquo;t another person to manage. They&rsquo;re
            a trained professional running your Home&nbsp;OS across every category
            of your household:
          </p>
          <div className="scope-grid">
            <span>Daily home resets</span>
            <span>Cleaning &amp; laundry</span>
            <span>Cooking &amp; meal prep</span>
            <span>Grocery &amp; restocking</span>
            <span>Childcare support</span>
            <span>Pet care</span>
            <span>Errands &amp; returns</span>
            <span>Organization &amp; decluttering</span>
            <span>Home maintenance</span>
            <span>Vehicle care</span>
            <span>Outdoor &amp; yard care</span>
            <span>Mail &amp; packages</span>
            <span>Calendar &amp; scheduling</span>
            <span>Gift management</span>
            <span>Guest management</span>
            <span>Travel management</span>
            <span>Seasonal transitions</span>
            <span>Technology management</span>
          </div>
          <p style={{ marginTop: 28 }}>
            Already have a cleaner? They skip housekeeping and focus on
            management. Have a nanny? They become the backup and handle
            everything the nanny doesn&rsquo;t. The role molds to what your
            family actually needs. We&rsquo;ll tailor it on your call.
          </p>
          <div className="pullquote">
            Most families start at 20&nbsp;hours a week, the threshold
            where you actually feel the difference. You can scale up anytime,
            flex seasonally, and at 40&nbsp;hours your household runs itself
            completely.
          </div>
        </div>
      </section>

      {/* Vetting deep dive */}
      <section id="how-we-find-them" className="sect rv">
        <div className="narrow">
          <div className="label">How we find your person</div>
          <h2>This person will be in your home. You need to trust the process behind them.</h2>
          <p style={{ fontSize: "0.88rem", color: "var(--text-soft)", marginBottom: 8 }}>
            The vetting methodology, the scoring system, and the background
            check protocol were built through years of trial and error in
            professional staffing before being brought to households.
          </p>
          <p>
            Every candidate is measured against three non-negotiables:
          </p>
          <div className="vetting-pillars">
            <div className="vetting-pillar rv rv-d1">
              <h3>Proven experience</h3>
              <p>
                Candidates who have done this job for multiple families, or high performers
                with strong transferable experience. We verify their track record with previous
                families and employers.
              </p>
            </div>
            <div className="vetting-pillar rv rv-d2">
              <h3>Values alignment</h3>
              <p>
                When your house manager walks in, you should feel like that person naturally
                belongs there. The energy shouldn&rsquo;t feel off. This is someone in your
                most private space, around your children, embedded in your daily life.
              </p>
            </div>
            <div className="vetting-pillar rv rv-d3">
              <h3>Safety</h3>
              <p>
                An eight-point background check covering county, state, federal, and national
                criminal searches, sex offender registry, global watchlist screening, SSN
                identity trace, and motor vehicle report. The most comprehensive check available.
              </p>
            </div>
          </div>
          <p style={{ marginTop: 28 }}>
            We go through hundreds of applicants every day. Before our team
            ever looks at a candidate, Ava, our AI agent trained on what makes
            an incredible house manager, screens their resume, application,
            and cover letter. Only the strongest candidates continue to a
            second round of knockout questions from Ava. Only then does our
            team interview those candidates to identify the best fits to
            endorse to you.
          </p>
          <p>
            You only meet pre-vetted finalists on a short call. You choose
            who to invite for a trial day, then a trial week. You decide
            based on a lived experience of what it&rsquo;s like to be
            supported by this person, not a r&eacute;sum&eacute;.
          </p>
          <p>
            Privacy is built into every step. We sign an NDA with you before
            we begin. Every candidate who enters the process signs one too.
            And once you choose your house manager, they sign a separate NDA
            as part of their employment. Candidates don&rsquo;t learn your
            address until they&rsquo;ve passed pre-vetting, completed
            interviews with you, cleared their background check, and had
            their references verified. Only then are they invited for a
            trial day.
          </p>
          <div className="pullquote">
            Your privacy is protected at every stage. You never commit to
            someone without seeing them in action first.
          </div>
        </div>
      </section>

      {/* Journey — two-phase timeline */}
      <section id="timeline" className="sect dark2 rv">
        <div className="container" style={{ textAlign: "center" }}>
          <div className="q-callout light">Families ask: &ldquo;How long does this take?&rdquo;</div>
          <div className="label">
            What happens after your call
          </div>
          <h2>From First Call to Autopilot</h2>
          <p
            style={{
              color: "var(--text-muted)",
              maxWidth: 480,
              margin: "0 auto 12px",
            }}
          >
            Two phases that run in parallel. While we build your system,
            we&rsquo;re already sourcing candidates.
          </p>

          <p style={{ color: "var(--text-soft)", fontSize: "0.8rem", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 4, marginTop: 28 }}>
            Phase 1: Build your system (21&nbsp;days)
          </p>
          <div className="journey-steps">
            <div className="j-step">
              <div className="j-dot active" />
              <h3>Discovery Call</h3>
              <p>You&rsquo;re here. We learn about your family and fit.</p>
            </div>
            <div className="j-step">
              <div className="j-dot" />
              <h3>Intake + First Drafts</h3>
              <p>
                You complete a short intake. Before our first working call, we
                already have drafts built from what you shared. Sourcing
                begins here too.
              </p>
            </div>
            <div className="j-step">
              <div className="j-dot" />
              <h3>Build + Review</h3>
              <p>
                Role design, hiring toolkit, home manual, onboarding system.
                Revisions happen between your intake call and your final
                presentation call. Everything customized to your family.
              </p>
            </div>
            <div className="j-step">
              <div className="j-dot" />
              <h3>Delivered</h3>
              <p>
                You own a complete system. If you find someone on your own
                before we present candidates, no placement fee. That&rsquo;s
                the contingency model. Most families let us finish the search.
              </p>
            </div>
          </div>

          <p style={{ color: "var(--text-soft)", fontSize: "0.8rem", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 4, marginTop: 36 }}>
            Phase 2: We find your person (runs in parallel)
          </p>
          <div className="journey-steps">
            <div className="j-step">
              <div className="j-dot" />
              <h3>Source + Screen</h3>
              <p>
                Ava screens every application, then knockout questions for the
                strongest. Our team interviews those who pass. You meet
                pre-vetted finalists.
              </p>
            </div>
            <div className="j-step">
              <div className="j-dot" />
              <h3>Trial + Place</h3>
              <p>
                Background check and references verified first. Then trial day,
                trial week. You decide based on lived experience. You only pay
                for placement when you&rsquo;re happy.
              </p>
            </div>
            <div className="j-step">
              <div className="j-dot" />
              <h3>Onboard + Autopilot</h3>
              <p>
                Your house manager walks in with full context from the Home&nbsp;OS.
                Day one doesn&rsquo;t feel like day one. Your home runs.
              </p>
            </div>
          </div>

          <p
            style={{
              color: "var(--text-muted)",
              fontSize: "0.85rem",
              maxWidth: 540,
              margin: "28px auto 0",
              lineHeight: 1.7,
            }}
          >
            Phase&nbsp;1 takes 21&nbsp;days and about 1&ndash;3&nbsp;hours
            of your time. Because sourcing runs in parallel, most families
            are fully placed within 30&ndash;60&nbsp;days total. We do the
            heavy lifting.
          </p>
        </div>
      </section>

      {/* Ongoing support — after onboarding */}
      <section className="sect rv">
        <div className="narrow">
          <div className="label">After placement</div>
          <h2>Built to make you independent</h2>
          <p>
            Your house manager is your employee. You pay them directly. To
            make that effortless, we connect you with a preferred partner
            who handles payroll, taxes, and workers&rsquo;&nbsp;comp. One phone
            call, a few questions, and you&rsquo;re set up.
          </p>
          <p>
            Before your house manager starts, we train them on the
            Home&nbsp;OS so they can update routines, preferences, and your
            home manual themselves. You&rsquo;re never reliant on us to keep
            things running.
          </p>
          <div className="pullquote">
            The system is yours. The manual is yours. Your house manager
            knows how to run it. That&rsquo;s the whole point.
          </div>
        </div>
      </section>

      {/* Replacement guarantee — #1 objection killer */}
      <section id="if-they-leave" className="sect rv">
        <div className="narrow" style={{ textAlign: "center" }}>
          <div className="q-callout">Families ask: &ldquo;What happens if my house manager leaves?&rdquo;</div>
          <div className="label">Built-in protection</div>
          <h2>What happens if your house manager leaves</h2>
          <p style={{ color: "var(--text-soft)" }}>
            We replace them at no additional cost within your guarantee window,
            up to 90&nbsp;days depending on your package. No re-placement fee.
            No new setup charge. And because your Home&nbsp;OS already has
            everything documented, your new house manager steps in with full
            context from day one. You&rsquo;re not starting over. The system
            protects you.
          </p>
          <div className="guarantee-badges">
            <div className="g-badge">
              <span className="g-badge-icon">90</span>
              <span className="g-badge-text">Up to 90-day replacement guarantee</span>
            </div>
            <div className="g-badge">
              <span className="g-badge-icon">$0</span>
              <span className="g-badge-text">No re-placement fees</span>
            </div>
            <div className="g-badge">
              <span className="g-badge-icon">&#10003;</span>
              <span className="g-badge-text">Home OS protects continuity</span>
            </div>
          </div>
        </div>
      </section>


      {/* Retention — why they stay */}
      <section className="sect dark rv">
        <div className="narrow">
          <div className="label">Why they stay</div>
          <h2>The #1 reason house managers leave any position? Part-time work.</h2>
          <p>
            When someone is working 20&nbsp;hours for one family, they&rsquo;re
            always one full-time offer away from leaving. That&rsquo;s the
            reality of this role across the entire industry, and it&rsquo;s
            the single biggest retention risk.
          </p>
          <p>
            HUM solves this structurally. When your house manager has availability
            beyond your hours, we work to place them with a second family nearby,
            securing full-time work for them without adding anything to your plate.
            Because our marketing is concentrated in local markets, this happens
            often. When it does, they&rsquo;re not looking elsewhere because their
            income is already whole.
          </p>
          <div className="pullquote">
            The Home&nbsp;OS gives them clarity. The onboarding builds a real
            relationship with your family. And the full-time income means
            they&rsquo;re not one job posting away from leaving. Retention
            isn&rsquo;t luck. It&rsquo;s structure.
          </div>
        </div>
      </section>

      {/* Joy interlude — the point of all of this */}
      <section className="sect warm rv">
        <div className="narrow" style={{ textAlign: "center" }}>
          <p className="joy-interlude">
            All of that, the system, the vetting, the guarantees, the
            structure, exists so that on a random Tuesday night, you&rsquo;re
            not thinking about any of it. You&rsquo;re on the couch. The house is
            quiet. Dinner happened. The kids are asleep. And you&rsquo;re just&hellip;
            there. Not planning. Not catching up. Just there.
          </p>
        </div>
      </section>

      {/* The investment — two-phase pricing */}
      <section id="the-investment" className="sect dark2 rv">
        <div className="narrow">
          <div className="q-callout light">Families ask: &ldquo;How much does this actually cost?&rdquo;</div>
          <div className="label" style={{ textAlign: "center" }}>The investment</div>
          <h2 style={{ textAlign: "center" }}>Two phases, fully transparent</h2>
          <p style={{ textAlign: "center", color: "var(--text-soft)", maxWidth: 520, margin: "0 auto 32px" }}>
            The process has two phases that run in parallel. Here&rsquo;s
            exactly what each one costs.
          </p>
          <div className="pricing-frame">
            <div className="pricing-part">
              <div className="pricing-part-label">Phase 1: Your hiring system and home manual</div>
              <div className="pricing-part-range">$3,000</div>
              <p>
                Everything you need to hire the right person and set them up
                to succeed. Role design, hiring toolkit, home manual, onboarding
                system, and compliance. All built around your family, not a
                template. Two calls, revisions in between, 21&nbsp;days.
              </p>
              <p style={{ fontSize: "0.88rem", color: "var(--text-muted)", marginBottom: 0 }}>
                You own everything we build. We start finding your person
                by default. If you find someone before we do, just let us know.
              </p>
            </div>
            <div className="pricing-part">
              <div className="pricing-part-label">Phase 2: We find your person</div>
              <div className="pricing-part-range">$3,500 &ndash; $4,500</div>
              <p>
                Contingency placement. You only pay if we successfully place
                someone you love. Full candidate sourcing, Ava screening,
                team interviews, trial day setup, background checks, and
                offer support. Backed by up to a 90-day replacement guarantee
                depending on your package.
              </p>
              <p style={{ fontSize: "0.88rem", color: "var(--text-muted)", marginBottom: 0 }}>
                If we don&rsquo;t find the right person, you don&rsquo;t pay
                for placement. The risk is on us.
              </p>
            </div>
          </div>
          <div className="employment-detail rv">
            <h3>W-2 or 1099. We help you choose.</h3>
            <p>
              Your house manager is your employee. You pay them directly.
              Most families save up to 50% on taxes with a W-2 household
              classification instead of 1099. We walk you through the pros
              and cons of both, then connect you with a preferred partner
              who handles payroll, taxes, workers&rsquo;&nbsp;comp, and
              compliance. One phone call, a few questions, and you&rsquo;re
              set up. No platform fees from HUM.
            </p>
          </div>
          <div style={{ marginTop: 40 }}>
            <CostCalculator embedded />
          </div>
        </div>
      </section>

      {/* What families didn't expect — experiential ROI */}
      <section id="the-roi" className="sect rv">
        <div className="narrow">
          <div className="q-callout">Families ask: &ldquo;Is it actually worth it?&rdquo;</div>
          <div className="label">What families tell us they didn&rsquo;t expect</div>
          <h2>The part no one sees coming</h2>
          <p>
            The calculator shows you the financial math. But when we check in
            with families a few months in, the money is never what they talk
            about. It&rsquo;s the stuff they didn&rsquo;t know they were
            missing:
          </p>
          <div className="roi-grid">
            <div className="roi-item rv rv-d1">
              <h3>&ldquo;We actually eat at home now&rdquo;</h3>
              <p>
                Groceries ordered, meals prepped, kitchen ready. Families tell
                us they spend far less on delivery and takeout once meals are
                handled. But the real shift is dinner becomes something you
                enjoy again.
              </p>
            </div>
            <div className="roi-item rv rv-d2">
              <h3>&ldquo;Saturday feels like Saturday again&rdquo;</h3>
              <p>
                No errands, no deep cleaning, no &ldquo;we should really
                organize the garage.&rdquo; The house ran all week. Saturday
                morning pancakes happen. Spontaneous trips to the park happen.
                The weekend is actually free.
              </p>
            </div>
            <div className="roi-item rv rv-d3">
              <h3>&ldquo;I&rsquo;m not two places at once anymore&rdquo;</h3>
              <p>
                At work, you&rsquo;re not thinking about the plumber. At
                home, you&rsquo;re not thinking about tomorrow&rsquo;s list.
                That split-brain feeling, half here, half somewhere
                else. It just goes away.
              </p>
            </div>
            <div className="roi-item rv rv-d4">
              <h3>&ldquo;We talk about things that aren&rsquo;t logistics&rdquo;</h3>
              <p>
                No more &ldquo;Did you call the electrician?&rdquo; or
                &ldquo;Can you grab milk?&rdquo; When the household runs
                itself, you find out your partner is actually pretty fun to
                be around.
              </p>
            </div>
            <div className="roi-item rv rv-d1">
              <h3>&ldquo;I go to the gym again&rdquo;</h3>
              <p>
                The first thing that got cut when life got heavy. Most
                families say it comes back within the first month. Not
                because they found the time. Because the time was
                already there.
              </p>
            </div>
            <div className="roi-item rv rv-d2">
              <h3>&ldquo;She knew before I did&rdquo;</h3>
              <p>
                Month one feels like relief. By month two, your house
                manager is anticipating what you need before you think
                of it. The system gets smarter. The
                home gets lighter.
              </p>
            </div>
          </div>
          <div className="pullquote" style={{ marginTop: 40 }}>
            The families who hesitate on the cost are usually the ones who,
            three months later, say they should have done it years ago.
          </div>
        </div>
      </section>

      {/* From families who've been here — social proof */}
      <section className="sect warm rv">
        <div className="container">
          <div className="narrow" style={{ textAlign: "center" }}>
            <div className="label">From families who&rsquo;ve been here</div>
            <h2>In their words</h2>
          </div>
          <div className="grid-2col" style={{ maxWidth: 720, margin: "32px auto 0" }}>
            <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 8, padding: 24 }}>
              <p style={{ fontSize: "0.95rem", color: "var(--text)", fontStyle: "italic", marginBottom: 8 }}>
                &ldquo;Life now feels calmer, more organized, and honestly just easier.
                Having the right house manager in place has taken a huge weight off
                our shoulders.&rdquo;
              </p>
              <p style={{ fontSize: "0.8rem", color: "var(--text-muted)", marginBottom: 0 }}>Tommy &amp; Suzi</p>
            </div>
            <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 8, padding: 24 }}>
              <p style={{ fontSize: "0.95rem", color: "var(--text)", fontStyle: "italic", marginBottom: 8 }}>
                &ldquo;I&rsquo;ve reclaimed 10-15 hours per week of my time.
                I&rsquo;m not sure how I ever got by without the help of a home
                manager now.&rdquo;
              </p>
              <p style={{ fontSize: "0.8rem", color: "var(--text-muted)", marginBottom: 0 }}>Mahti</p>
            </div>
            <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 8, padding: 24 }}>
              <p style={{ fontSize: "0.95rem", color: "var(--text)", fontStyle: "italic", marginBottom: 8 }}>
                &ldquo;I told my kids last weekend how much happier and peaceful I
                feel with Patti&rsquo;s amazing support to&nbsp;us!&rdquo;
              </p>
              <p style={{ fontSize: "0.8rem", color: "var(--text-muted)", marginBottom: 0 }}>Amy</p>
            </div>
            <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 8, padding: 24 }}>
              <p style={{ fontSize: "0.95rem", color: "var(--text)", fontStyle: "italic", marginBottom: 8 }}>
                &ldquo;She&rsquo;s my person. I&rsquo;d like her to be ours
                immediately. And forever.&rdquo;
              </p>
              <p style={{ fontSize: "0.8rem", color: "var(--text-muted)", marginBottom: 0 }}>Sandi</p>
            </div>
          </div>
        </div>
      </section>

      {/* More questions — Tier 3 */}
      <section id="more-questions" className="sect dark rv">
        <div className="narrow">
          <div className="label" style={{ textAlign: "center" }}>Still have questions?</div>
          <h2 style={{ textAlign: "center" }}>Other things families ask</h2>
          <p style={{ textAlign: "center", color: "var(--text-soft)", marginBottom: 36 }}>
            Everything above covers the big picture. Here are the practical details families want to know.
          </p>
          <FAQ />
        </div>
      </section>

      {/* Closing */}
      <section className="close-sect rv">
        <div className="narrow">
          <h2>You already booked the call</h2>
          <p>
            Here&rsquo;s what happens next: a 30-minute conversation with
            someone who genuinely wants to understand your family. What&rsquo;s
            not working, what you&rsquo;ve already tried, and what it would
            look like if your home just&hellip; ran. No pitch. No pressure.
            If HUM isn&rsquo;t the right fit, we&rsquo;ll tell you. And if
            you have more questions, we&rsquo;ll stay on as long as you need.
          </p>
          <p>
            The call itself feels different. Not like a sales conversation.
            More like talking to someone who actually gets it. The weight,
            the invisible work, the guilt about wanting help. For the first
            time, someone offering a real solution instead of another band-aid.
          </p>

          {/* Prep reinforcement */}
          <div style={{ margin: "36px 0", padding: "28px 32px", background: "var(--gold-bg)", border: "1px solid rgba(184,146,107,0.15)", borderRadius: 8 }}>
            <h3 style={{ textAlign: "center", marginBottom: 16 }}>Before your call, remember</h3>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <p style={{ fontSize: "0.9rem", color: "var(--text-soft)", marginBottom: 0 }}>
                <strong>1.</strong> Think about what&rsquo;s not working, what you&rsquo;ve tried, and what &ldquo;great&rdquo; looks like.
              </p>
              <p style={{ fontSize: "0.9rem", color: "var(--text-soft)", marginBottom: 0 }}>
                <strong>2.</strong> Bring your questions. We&rsquo;ll stay on as long as you need.
              </p>
            </div>
          </div>

          <div className="partner-callout">
            <h3>Bring your partner if you can</h3>
            <p>
              The families that move fastest are the ones where both decision-makers
              hear it firsthand. If they can join, even for part of the call,
              it saves weeks.
            </p>
          </div>
          <p>
            You&rsquo;ve been carrying this for a long time. You booked this
            call because something told you it was time to stop. Trust that.
            The life you&rsquo;ve been reading about on this page, it
            starts with this conversation.
          </p>
          <p className="signoff">We&rsquo;ll see you there.</p>
        </div>
      </section>

      <PrecallFooter />
    </div>
  );
}
